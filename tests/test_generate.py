...
class TestBuildEvent:
    ...
    def test_build_event_basic(self):
        ...
        # dtstart が date 型（datetime ではない）であることを確認 = 終日イベント
        dtstart = event.get('dtstart').dt
        assert isinstance(dtstart, date) and not isinstance(dtstart, datetime)
        # 日付が一致することを確認
        assert dtstart == dt.date()
        assert str(event.get('uid')) == uid
...
class TestAddHolidayEvents:
    ...
    def test_add_holiday_events_with_holidays(self):
        ...
        event_dates = {event.get('dtstart').dt for event in c.walk('VEVENT')}
    ...
    def test_add_holiday_events_with_invalid_date(self):
        ...
        add_holiday_events(c, calendar_list)
        assert len(list(c.walk('VEVENT'))) == 0
        ...
        try:
            ...
            def mock_open(file_path, mode='r', **kwargs):
                if file_path == "japan-all-stocks.ics":
                    return original_open(tmp_path, mode, **kwargs)
            ...
            try:
                ...
                with open(tmp_path, 'rb') as f:
                    ...
                    assert len(events) >= 0, "カレンダーにイベントが含まれていません"
